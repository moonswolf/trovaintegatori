        ...
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2 overflow-x-auto flex-1 min-w-0">
            ...
            <div className="flex gap-2">
              {products.map(p => p && (
                <div key={p.id} className="flex items-center gap-1 bg-slate-50 rounded-lg px-2 py-1 min-w-0">
                  ...
                  <span className="text-xs text-slate-700 truncate max-w-[80px] md:max-w-[120px]">{p.name}</span>
                  <button
                    onClick={() => removeFromCompare(p.id)}
                    className="text-slate-400 hover:text-red-500 flex-shrink-0 text-xs ml-1"
                  >
                    ✕
                  </button>
                </div>
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            <button
              ...
              className="bg-emerald-600 text-white px-4 py-2 rounded-lg font-semibold text-sm whitespace-nowrap hover:bg-emerald-700 transition disabled:bg-slate-300 disabled:cursor-not-allowed"
            >
              Confronta ({compareIds.length})
            </button>
            <button
